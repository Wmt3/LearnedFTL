...
static inline bool valid_ppa(struct ssd *ssd, struct ppa *ppa) {
  ...
  if (ch >= 0 && ch < spp->nchs && lun >= 0 && lun < spp->luns_per_ch &&
      pl >= 0 && pl < spp->pls_per_lun && blk >= 0 && blk < spp->blks_per_pl &&
...
static uint64_t ssd_advance_status(struct ssd *ssd, struct ppa *ppa,
  ...
  int c = ncmd->cmd;
  uint64_t cmd_stime =
      (ncmd->stime == 0) ? qemu_clock_get_ns(QEMU_CLOCK_REALTIME) : ncmd->stime;
  uint64_t nand_stime;
  struct ssdparams *spp = &ssd->sp;
  struct nand_lun *lun = get_lun(ssd, ppa);
  uint64_t lat = 0;
...
static uint64_t ssd_read(struct ssd *ssd, NvmeRequest *req) {
  ...
  for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
    ...
    if (!mapped_ppa(&ppa) || !valid_ppa(ssd, &ppa)) {
      ...
      // lpn); printf("Invalid ppa,ch:%d,lun:%d,blk:%d,pl:%d,pg:%d,sec:%d\n",
